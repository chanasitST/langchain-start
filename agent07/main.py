...
def create_knowledge_base(path: str):
    """Creates a dummy text file with content for the study plan."""
    content = """
...
def main():
    print("\n🧠 Starting Day 7: Study Plans (Verification)...\n")
    ...
    try:
        ...
        topic = "Mastering LangGraph"
        duration = "2 hours"
        plan = generate_study_plan(topic, duration, retriever)
        ...
        for i, session in enumerate(plan.sessions, 1):
            print(f"\nSession {i}: {session.goal} ({session.total_duration_minutes} mins)")
            for item in session.items:
                print(f"   - {item.activity} ({item.duration_minutes}m)")
                if item.resources:
                    print(f"     Resources: {', '.join(item.resources)}")
            print("-" * 50)
